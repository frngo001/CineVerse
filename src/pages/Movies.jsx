...
const Movies = () => {
  ...
  useEffect(() => {
    loadMovies();
  }, [currentPage, selectedGenres, sortBy, yearFilter, selectedLanguage, selectedCountry]);
  ...
  }, [selectedGenres, sortBy, yearFilter, selectedLanguage, selectedCountry]);
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-16">
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm transition-colors duration-200 ${
                sortBy === 'popularity.desc' && selectedGenres.length === 0 && !yearFilter
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm transition-colors duration-200 ${
                sortBy === 'release_date.desc' && selectedGenres.length === 0 && !yearFilter
                  ? 'bg-red-600 text-white' 
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm transition-colors duration-200 ${
                sortBy === 'vote_average.desc' && selectedGenres.length === 0 && !yearFilter
                  ? 'bg-red-600 text-white' 
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
            ...
            <button
              onClick={() => {
                setSortBy('popularity.desc');
                setSelectedGenres([28]); // Action genre
                setYearFilter('');
              }}
              className={`px-4 py-2 rounded-full text-sm transition-colors duration-200 ${
                selectedGenres.includes(28)
                  ? 'bg-red-600 text-white' 
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
            ...
            <button
              onClick={() => {
                setSortBy('popularity.desc');
                setSelectedGenres([27]); // Horror genre
                setYearFilter('');
              }}
              className={`px-4 py-2 rounded-full text-sm transition-colors duration-200 ${
                selectedGenres.includes(27)
                  ? 'bg-red-600 text-white' 
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
        ...
        <div className="bg-[#2F2F2F] rounded-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            {/* Country Filter */}
            <div>
              <CountrySelector showLanguageInfo={true} />
            </div>
          ...
          {(selectedGenres.length > 0 || yearFilter || selectedLanguage || searchQuery || selectedCountry) && (
                  ...
                  <span className="bg-green-600 text-white px-3 py-1 rounded-full text-sm flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        // This will be removed or handled differently
                      }}
        ...
        <div className="flex justify-between items-center mb-6">
          <p className="text-gray-400">
            Page {currentPage} sur {totalPages} ({movies.length} films)
          </p>
          <div className="text-gray-400 text-sm">
            {sortBy === 'popularity.desc' && 'Triés par popularité'}
            {sortBy === 'vote_average.desc' && 'Triés par note'}
            {sortBy === 'release_date.desc' && 'Triés par date de sortie'}
            {sortBy === 'title.asc' && 'Triés par titre'}
            {sortBy === 'vote_count.desc' && 'Triés par nombre de votes'}
          </div>
        </div>
        ...
        {!loading && movies.length === 0 && (
          <div className="text-center py-20">
            <div className="text-white text-xl mb-4">Aucun film trouvé</div>
